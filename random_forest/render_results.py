...
def main(models, word_lengths):
    ...
    for path in models:
        ...
        with open(f"stats/{path}.json", 'r') as f:
            data = json.load(f)
    ...
    model_size = {
        p : os.path.getsize(f"models/{p}") for p in models
    }
    ...
    accuracy_overall.plot(x="model", y="model_size", kind="bar", title="Model size by model")
    plt.xticks(rotation=0)

    accuracy_overall.plot(x = "model", y = "accuracy", kind = "bar", title = "Overall accuracy by model")
    plt.xticks(rotation=0)
    plt.ylim(0, 1)
...
if __name__ == "__main__":
    models = [os.path.basename(x).split('.')[0] for x in os.listdir("stats")]
